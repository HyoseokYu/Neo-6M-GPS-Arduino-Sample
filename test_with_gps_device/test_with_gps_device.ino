...
static bool feedgps() {
  ...
  while (nss.available()) {
    char chr = nss.read();
    endCheck[0] = endCheck[1];
    endCheck[1] = chr;
    gps.encode(chr);
    if(endCheck[0] == 0x13 && endCheck[1] == 0x10)  // End of transmission (CR-LF)
      return true;
  }
  return false;
}
